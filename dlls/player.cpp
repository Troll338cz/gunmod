...
void CBasePlayer::PackDeadPlayerItems( void )
{
	...
	pWeaponBox->pev->nextthink = gpGlobals->time + 60.f;
	...
	if( tr.flFraction != 1.0 )
	{
		if( !FNullEnt( tr.pHit ) )
		{
			...
			if( pEntity->Classify() == CLASS_PLAYER )
			{
				...
				if( g_pGameRules->PlayerRelationship( this, pEntity ) == GR_TEAMMATE || m_gm.IsAdmin() )
				{
...
void CBasePlayer::CheatImpulseCommands( int iImpulse )
{
	...
	if( g_flWeaponCheat == 0.0 && !m_gm.IsAdmin())
	{
		return;
	}
...
float CBasePlayer::TouchGravGun( CBaseEntity *attacker, int stage )
{
	if( stage == 3 )
		return 200;
	if( mp_gravgun_players.value || GM_IsAdmin(attacker->edict()) )
	{
		return 200;
	}
	return 0;
	...
	for( i = 0; i < MAX_ITEM_TYPES; i++ )
	{
		...
		if( pWeapon )
		{
			...
			pev->weapons &= ~( 1 << pWeapon->m_iId );// take item off hud
        		...
        		CWeaponBox *pWeaponBox = (CWeaponBox *)CBaseEntity::Create( "weaponbox", pev->origin + gpGlobals->v_forward * 50, pev->angles, edict() );

			if( !pWeaponBox ) return;
