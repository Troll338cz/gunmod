...
void CBasePlayerItem::AttemptToMaterialize( void )
{
	...
	if( time == 0 )
	{
		if( mp_tolchock.value && !RANDOM_LONG(0,200) )
		{
			...
			tolchock->pev->spawnflags |= 64;
			tolchock->pev->model = MAKE_STRING( "models/xash/toilet.mdl" );
			tolchock->Spawn();
			tolchock->pev->rendermode = 1;
			tolchock->pev->renderamt = 255;
			tolchock->pev->rendercolor = Vector(1,1,1);
			tolchock->pev->velocity = Vector( RANDOM_LONG(100,1000), RANDOM_LONG(100,1000), RANDOM_LONG(100,1000) );
...
void CBasePlayerItem::DefaultTouch( CBaseEntity *pOther )
{
	...
	if( g_pGameRules->IsHideAndSeek() && !(this->pev->spawnflags & SF_NORESPAWN) )
		return;
