...
void CBottle::BottleTouch( CBaseEntity *pOther )
{
	...
	if( !pPlayer->m_gm.bSitted && pPlayer->pev->button & IN_DUCK && pOther->pev->absmin.z >= (pev->origin.z + pev->maxs.z -1) )
	{
		pPlayer->m_gm.bSitted = TRUE;
		UTIL_ShowMessage( "Congratulations!\nYou sat down on the bottle", pPlayer );
		char text[256] = "";

		if( pPlayer->pev->netname )
		{
			snprintf( text, sizeof(text), "   %s ^1sat down on the bottle\n", STRING( pPlayer->pev->netname ) );
			UTIL_LogPrintf( "\"%s<%i><%s>\" sat down on the bottle\n",
                                STRING( pPlayer->pev->netname ),
